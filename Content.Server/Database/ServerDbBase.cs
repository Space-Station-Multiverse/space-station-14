...
namespace Content.Server.Database
{
    public abstract class ServerDbBase
    {
        ...
        public async Task<PlayerRecord?> GetPlayerRecordByHWID(TypedHwid? hwID, CancellationToken cancel)
        {
            // if HWid is empty, then don't match any records based on it.
            // (Historically, that would be all linux/mac players before hwid support was added to them)
            if (hwID == null || hwID.Hwid.Length == 0)
                return null;
