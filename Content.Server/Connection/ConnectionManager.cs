        ...
        */
        ...
        private async Task NetMgrOnConnecting(NetConnectingArgs e)
        {
            ...
            if (deny != null)
            {
                ...
                var id = await _db.AddConnectionLogAsync(userId, e.UserName, addr, hwid, trust, reason, serverId, e.UserData.PublicKey);
            ...
            {
                await _db.AddConnectionLogAsync(userId, e.UserName, addr, hwid, trust, null, serverId, e.UserData.PublicKey);

                if (!ServerPreferencesManager.ShouldStorePrefs(e.AuthType))
                    return;

                await _db.UpdatePlayerRecordAsync(userId, e.UserName, addr, hwid, e.UserData.PublicKey);
            }
        }
            ...
            NetConnectingArgs e)
        {
            ...
            */
            ...
            // VPN Block (SSMV)
            // (In future, may want to refactor this to merge with the system wizden added after MV)
            ...
            } else // No need to run wizden VPN block if already using MV VPN block
            {
